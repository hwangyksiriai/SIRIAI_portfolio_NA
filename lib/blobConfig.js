...
/* initialConfig owns which categories exist and in what order; the stored copy
   ...
   pages with it. */
/* The admin edits a category's title and clips; everything else about it —
   which layout it uses, its nav label, its tag — belongs to the source. Taking
   the stored object whole meant a category could never change layout again,
   since the copy saved before the change would keep winning. */
function withContentFrom(base, stored) {
  if (!stored) return base;
  const out = { ...base };
  if (typeof stored.title === 'string') out.title = stored.title;
  if (Array.isArray(stored.regions)) {
    out.regions = stored.regions;
    delete out.clips;
  } else if (Array.isArray(stored.clips)) {
    out.clips = stored.clips;
    delete out.regions;
  }
  return out;
}
...
function withSourceCategories(stored) {
  ...
  for (const base of initialConfig.categories) {
    categories.push(withContentFrom(base, edited.get(base.id)));
    categories.push(
      ...stored.categories
